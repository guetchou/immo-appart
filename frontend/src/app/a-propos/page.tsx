...
function contactIcon(type?: string) {
  if (type === 'telephone') return Phone
  if (type === 'email') return Mail
  return MapPin
}

export default async function AProposPage() {
  const page = await getPageAPropos()
  const points = (page?.points_forts as string[] | null) ?? DEFAULT_POINTS
  const contacts = (page?.contact_items as typeof DEFAULT_CONTACT | null) ?? DEFAULT_CONTACT
      ...
      <main className="pt-[68px]" style={{ background: '#FBF8F4' }}>
        <div className="py-14 px-8" style={{ background: '#1A0E06' }}>
          <div className="max-w-[820px] mx-auto text-center">
            <div className="text-[11px] font-bold tracking-[2px] uppercase text-[#F09A55] mb-3">{(page?.hero_label as string) ?? 'Notre histoire'}</div>
            <h1 className="font-black text-white mb-4" style={{ fontSize: 'clamp(30px,4vw,46px)', fontFamily: 'var(--font-heading)', letterSpacing: '-1px' }}>
              {(page?.hero_titre as string) ?? 'Résidence NDOMBI'}
            </h1>
            <p className="text-[16px] leading-relaxed" style={{ color: 'rgba(255,255,255,.65)' }}>
              {(page?.hero_texte as string) ?? 'Appartements meublés haut de gamme à Foucks, Pointe-Noire — République du Congo.'}
            </p>
          </div>
        </div>
        ...
        <div className="max-w-[820px] mx-auto px-8 py-16 space-y-12">
          <section>
            <h2 className="font-black text-[#1A0E06] text-[24px] mb-4" style={{ fontFamily: 'var(--font-heading)' }}>{(page?.mission_titre as string) ?? 'Notre mission'}</h2>
            <p className="text-[#7A6550] leading-relaxed text-[15px]">
              {(page?.mission_texte as string) ?? 'La Résidence NDOMBI propose des appartements meublés entièrement équipés pour des séjours professionnels ou familiaux à Pointe-Noire. Nous offrons un service personnalisé, une confirmation rapide et des prestations hôtelières sans les contraintes d’un grand hôtel.'}
            </p>
          </section>

          <section>
            <h2 className="font-black text-[#1A0E06] text-[24px] mb-5" style={{ fontFamily: 'var(--font-heading)' }}>{(page?.points_titre as string) ?? 'Ce qui nous distingue'}</h2>
          ...
          <section className="bg-white rounded-2xl p-8" style={{ border: '1px solid #E5DDD4' }}>
            <h2 className="font-black text-[#1A0E06] text-[24px] mb-6" style={{ fontFamily: 'var(--font-heading)' }}>{(page?.contact_titre as string) ?? 'Nous contacter'}</h2>
            <div className="space-y-4">
              {contacts.map(({ type, texte }) => {
                ...
                <div key={texte} className="flex items-start gap-3">
                  ...
                  <span className="text-[14px] text-[#1C110A]">{texte}</span>
                </div>
              )})}
            </div>
          </section>
