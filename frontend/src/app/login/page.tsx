...
const list = (config: AuthEspaceConfig | null, key: string, fallback: string[]) =>
  Array.isArray(config?.[key]) ? (config[key] as unknown[]).filter((item): item is string => typeof item === 'string') : fallback
...
export default function LoginPage() {
  ...
  const loginAdvantages = list(config, 'login_avantages', [
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden flex-col justify-between p-12"
        ...
        <img
          src={s(config, 'login_image_url', 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=900&h=1200&fit=crop')}
        ...
        <div className="relative z-10">
          <h2 className="font-black text-white mb-4"
            ...
            {s(config, 'login_accroche_titre', 'Votre espace')}<br />
            <span style={{ color: '#F09A55' }}>{s(config, 'login_accroche_accent', 'privilégié')}</span>
          </h2>
          <p className="text-[15px] mb-8" style={{ color: 'rgba(255,255,255,.65)', lineHeight: 1.7 }}>
            {s(config, 'login_accroche_texte', 'Gérez vos réservations, consultez vos séjours passés et accédez à vos offres personnalisées.')}
          </p>
          <div className="flex flex-col gap-3">
            {loginAdvantages.map(f => (
      ...
      <div className="flex-1 flex flex-col justify-center px-8 py-12 bg-white lg:px-16 xl:px-24">
        ...
        <Link href="/" className="inline-flex items-center gap-2 text-[13px] font-medium mb-10 w-fit transition-colors hover:text-[#E07A2F]"
          style={{ color: '#7A6550' }}>
          <ArrowLeft size={15} /> {s(config, 'login_retour_label', "Retour à l'accueil")}
        </Link>
        ...
        <div className="mb-8">
          <h1 className="font-black text-[#1A0E06] mb-2"
            ...
            {s(config, 'login_titre', 'Bienvenue')}
          </h1>
          <p className="text-[#7A6550] text-[15px]">
            {s(config, 'login_sous_titre', 'Connectez-vous à votre espace Résidence NDOMBI')}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <div>
            <label className="block text-[13px] font-semibold text-[#1A0E06] mb-2">
              {s(config, 'login_email_label', 'Adresse email')}
            </label>
            <div className="relative">
              ...
              <input
                ...
                placeholder={s(config, 'login_email_placeholder', 'votre@email.com')}
                autoComplete="email"
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[13px] font-semibold text-[#1A0E06]">
                {s(config, 'login_password_label', 'Mot de passe')}
              </label>
              <Link href="/mot-de-passe-oublie" className="text-[12px] font-medium transition-colors hover:text-[#E07A2F]"
                style={{ color: '#7A6550' }}>
                {s(config, 'login_password_oublie_label', 'Mot de passe oublié ?')}
              </Link>
            </div>
            <div className="relative">
          ...
          >
            {loading
              ? <span className="flex items-center justify-center gap-2"><Loader2 size={16} className="animate-spin" /> {s(config, 'login_loading_label', 'Connexion…')}</span>
              : s(config, 'login_submit_label', 'Se connecter')
            }
          </button>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-[12px] text-[#7A6550] font-medium">{s(config, 'login_separator_label', 'ou continuer avec')}</span>
            <div className="flex-1 h-px bg-[#E5DDD4]" />
          </div>
          ...
          <p className="text-center text-[14px] text-[#7A6550]">
            {s(config, 'login_no_account_label', 'Pas encore de compte ?')}{' '}
            <Link href="/inscription" className="font-bold transition-colors hover:text-[#B85E18]"
              style={{ color: '#E07A2F' }}>
              {s(config, 'login_create_account_label', 'Créer un compte')}
            </Link>
          </p>
        </form>
      </div>
